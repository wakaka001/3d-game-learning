...
public class DiskThrower : MonoBehaviour
{
    ...
    public void sendDisk2(List<GameObject> usingDisks)
    {
        ...
        for (int i = 0; i < usingDisks.Count; i++)
        {
            ...
            usingDisks[i].GetComponent<Renderer>().material.color = GetRandomColor();
            if(usingDisks.Count%2 == 1) usingDisks[i].transform.position = new Vector3(startPosition.x, startPosition.y + i+8, startPosition.z+8);
            else usingDisks[i].transform.position = new Vector3(startPosition.x, startPosition.y + i + 8, startPosition.z+8);
            GameScenceController.getGSController().setScenceStatus(ScenceStatus.shooting);
        }
    }
    ...
    private void Update()
    {
        ...
        if (GameScenceController.getGSController().mode == "sport")
        {
            ...
            for (int i = 0; i < usingDisks.Count; i++)
            {
                ...
                if(usingDisks.Count%2 == 1) usingDisks[i].transform.Translate(Vector3.right * 8f * Time.deltaTime);
                else usingDisks[i].transform.Translate(Vector3.left  * 8f * Time.deltaTime);
                }
